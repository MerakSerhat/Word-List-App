 ...
 * @author mzechner
 * @author Nathan Sweet */
public class SlowScrollPane extends WidgetGroup {
    ...
    public SlowScrollPane (@Null Actor widget, com.badlogic.gdx.scenes.scene2d.ui.ScrollPane.ScrollPaneStyle style) {
        ...
        addCaptureListener(new InputListener() {
            ...
            public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (getStage() != null) getStage().setScrollFocus(SlowScrollPane.this);
        ...
        addListener(new InputListener() {
            public boolean scrolled (InputEvent event, float x, float y, int amount) {
                if(dtSlow > 0.05f) {
                    dtSlow = 0;
                    setScrollbarsVisible(true);
                    System.out.println(getMouseWheelY() * amount / 5);
                    if (scrollY)
                        setScrollY(amountY + getMouseWheelY() * amount / 5);
                    else if (scrollX) //
                        setScrollX(amountX + getMouseWheelX() * amount / 5);
                    else
                        return false;
                    return true;
                }
